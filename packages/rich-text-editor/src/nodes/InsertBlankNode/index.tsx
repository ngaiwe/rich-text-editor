...
export type SerializedBlankNode = Spread<InsertBlankPayload, SerializedLexicalNode>;

// 插空组件
const InsertBlankComponent = React.lazy(() => import('./InsertBlankComponent'));

function convertBlankElement(): null | DOMConversionOutput {
...
export class InsertBlankNode extends DecoratorNode<JSX.Element> {
  ...
  static importJSON(): InsertBlankNode {
    const node = $createInsertBlankNode({});
    ...
    return {
      span: (node: Node) => {
        const status =
          ...
          node.getAttribute(CUSTOMIZE_ATTRIBUTE_TYPE) === InsertBlankNodeZhTagType;
  ...
  decorate(): JSX.Element {
    return (
      <Suspense fallback={null}>
        <InsertBlankComponent nodeKey={this.getKey()} index={this.__serialIndex} />
      </Suspense>
    );
  }
}

export function $createInsertBlankNode({ key }: InsertBlankPayload): InsertBlankNode {
  return $applyNodeReplacement(new InsertBlankNode(key));
}

export function $isInsertBlankNode(node: LexicalNode | null | undefined): node is InsertBlankNode {
