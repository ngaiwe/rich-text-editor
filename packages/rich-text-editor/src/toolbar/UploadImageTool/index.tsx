...
import { IMAGE_TYPE_WHITE, MAX_IMAGE_SIZE, MAX_IMAGE_SIZE_TEXT } from '../../config/GlobalConstant';
...
const UploadImageTool = (props: any) => {
  ...
  const params = useMemo(() => Object.assign({}, UploadPropsStatic, props), [props]);
  ...
  const beforeUploadCallback = useCallback((file: any) => {
    ...
    if (!IMAGE_TYPE_WHITE.includes(file?.type || '')) {
      message.warning(`图片格式有误 ${file?.type}, 仅支持 ${IMAGE_TYPE_WHITE.join(',')} 格式`);
