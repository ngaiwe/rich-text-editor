    ...
    if (activeCell) {
      ...
      if (draggingDirection && mouseCurrentPos && tableRect) {
        if (isHeightChanging(draggingDirection)) {
          ...
          styles[draggingDirection].top = `${window.scrollY + mouseCurrentPos.y}px`;
          styles[draggingDirection].height = '3px';
          styles[draggingDirection].width = `${tableRect.width}px`;
        } else {
          styles[draggingDirection].top = `${window.scrollY + tableRect.top}px`;
          styles[draggingDirection].left = `${window.scrollX + mouseCurrentPos.x}px`;
...
export default function TableCellResizerPlugin(): null | ReactPortal {
  ...
  return useMemo(
    () =>
      isEditable
        ? (createPortal(<TableCellResizer editor={editor} />, document.body) as ReactPortal)
        : null,
    [editor, isEditable],
  );
}
