...
export class EditorParser {
  ...
  private __parserTag(child: Element): ReactElement {
    try {
      ...
      const reactComponent = doStrategy(
        child,
        attribute[ZHTAGTYPE],
        this.__reactComponentMapping
      )
  ...
  public getReactNodeMapping(
    ...
    if (key) {
      return reactDoms.filter(
        reactDom => reactDom?.props?.name && reactDom.props.name === key
      )
    }
